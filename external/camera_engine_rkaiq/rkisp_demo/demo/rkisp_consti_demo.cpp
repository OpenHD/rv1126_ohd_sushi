#ifndef RKISP_CONSTI_DEMO
#define RKISP_CONSTI_DEMO
/*
...
{
    DBG("Consti10 process_image begin\n");
    ...
    DBG("Consti10 process_image end\n");
}
...
static int read_frame(demo_context_t *ctx)
{
    ...
    const auto buffTimeStamp=timevalToDuration(buf.timestamp);
    const uint64_t buffTimeStampNs=buffTimeStamp.count();
    const auto delay=std::chrono::steady_clock::now()- timevalToTimePointSteadyClock(buf.timestamp);
    const float delayMs=std::chrono::duration_cast<std::chrono::microseconds>(delay).count()/1000.0f;

    DBG("Consti10 read_frame sequence:%d timestamp:%lld delay:%f ms\n",buf.sequence,buffTimeStampNs,delayMs);
