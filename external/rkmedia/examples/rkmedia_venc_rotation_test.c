...
int main(int argc, char *argv[]) {
  ...
  RK_U64 cnt = 0;
  VENC_ROTATION_E ro_para = VENC_ROTATION_0;
  while (!quit) {
    cnt++;
    if ((cnt % 10) == 0) {
      if (ro_para == VENC_ROTATION_0) {
        ro_para = VENC_ROTATION_90;
      } else if (ro_para == VENC_ROTATION_90) {
        ro_para = VENC_ROTATION_180;
      } else if (ro_para == VENC_ROTATION_180) {
        ro_para = VENC_ROTATION_270;
      } else {
        ro_para = VENC_ROTATION_0;
      }
      RK_MPI_VENC_SetRotation(0, ro_para);
      printf(">>>>>>>>>rotation %d\n", ro_para);
    }
    usleep(500000);
  }
