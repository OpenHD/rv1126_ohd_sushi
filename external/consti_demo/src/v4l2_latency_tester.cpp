...
static void new_frame_callback(v4l2_buffer& v4L2Buffer,buffer *buffers){
    const auto timestamp=timevalToTimePointSystemClock(buf.timestamp);
    const auto delay=std::chrono::system_clock::now()-timestamp;
    std::cout<<"Buff idx:"<<buf.index<<" Sequence:"<<buf.sequence<<" Delay:"<<std::chrono::duration_cast<std::chrono::microseconds>(delay).count()/1000.0f<<" ms timestamp: "<<timestamp.time_since_epoch().count()<<"\n";
...
int main(int argc, char **argv)
{
    ...
    while (1)
    {
        ...
        const auto timestamp=timevalToTimePointSystemClock(buf.timestamp);
        const auto delay=std::chrono::system_clock::now()-timestamp;
        std::cout<<"Buff idx:"<<buf.index<<" Sequence:"<<buf.sequence<<" Delay:"<<std::chrono::duration_cast<std::chrono::microseconds>(delay).count()/1000.0f<<" ms"
